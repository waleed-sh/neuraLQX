...
class TriangularLatticeGraph(Graph):
    r"""
    Triangular lattice graph on an :math:`m\times n` patch, optionally periodic.

    This graph is a two-dimensional lattice obtained from a rectangular grid by adding diagonal
    connections so that elementary faces become triangles (as in
    :func:`networkx.generators.triangular_lattice_graph`). The ``periodic`` flag enables periodic
    boundary conditions in the NetworkX construction.

    NetworkX uses coordinate-labelled vertices for this generator; neuraLQX relabels them to
    ...
    :param m: Lattice size parameter (as interpreted by the NetworkX generator).
    :param n: Lattice size parameter (as interpreted by the NetworkX generator).
    :param periodic: If True, impose periodic boundary conditions as supported by NetworkX.
    ...
    def __init__(
        ...
        *,
        ...
        # create a temporary networkx triangular lattice graph
        _tmp_nx = nx.generators.triangular_lattice_graph(m, n, periodic=periodic)
