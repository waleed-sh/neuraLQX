...
class HypercubeGraph(Graph):
    r"""
    :math:`N`-dimensional hypercube graph :math:`Q_N`.

    Vertices correspond to bit-strings of length :math:`N`, i.e. :math:`\{0,1\}^N`, and two vertices
    are adjacent iff they differ in exactly one bit (Hamming distance one). Consequently:

    .. math::

        |V| = 2^N,\qquad |E| = N\,2^{N-1}.

    The graph is constructed from :func:`networkx.generators.hypercube_graph`, then relabelled to
    consecutive integer vertex labels before being passed to :class:`~neuralqx.graph.graph.Graph`.
    If ``non_planar=True``, vertices are relabelled to 3-tuples and may be given a random spatial
    embedding.

    :param N: Hypercube dimension.
    ...
    def __init__(
        ...
        *,
        ...
        # create a temporary networkx N-dimensional hypercube graph
        _tmp_nx = nx.generators.hypercube_graph(N)

        # relabel the networkx edges
