...
if TYPE_CHECKING:
    from .numba import MarkedLocalOperator


@register_pytree_node_class
class MarkedLocalOperatorJax(LocalOperatorBase, DiscreteJaxOperator):
    """
    A JAX-compatible MarkedLocalOperator which works with JAX sharding. It does not implement
    ...
    def _numba_operator(self) -> "MarkedLocalOperator":
        """
        ...
        if not hasattr(self, "_cached_numba_op"):
            ...
            # construct the concrete MarkedLocalOperator and cache
            self._cached_numba_op = self.to_numba_operator()

        # return the cache
        return self._cached_numba_op

    def to_numba_operator(self) -> "MarkedLocalOperator":
        """
    ...
    def _setup(self, force: bool = False):
        """
        Setup up the JAX-compatible MarkedLocalOperator. This is done once-per-object and the
    ...
    def _get_conn_padded(self, x):
        ...
        # Python callback executed on host (local shard only)

        # get the maximum number of connections and the expected dtype of the matrix elements
        max_conn = self._max_conn_size

        # MarkedLocalOperator always returns real valued fp64 dtype
        mel_dtype = np.float64
        ...
        def _py_kernel(x_np: np.ndarray):
            ...
            # ensure dtype matches spec, again, always fp64 for MarkedLocalOperator
            mels_np = mels_np.astype(np.float64, copy=False)
