...
from .field_helpers import FACTORY_DEFAULT, MISSING
...
def safe_array_equal(a: Any, b: Any) -> bool:
    ...
    if isinstance(a, (tuple, list)) and isinstance(b, type(a)) and len(a) == len(b):
        return all(safe_array_equal(x, y) for x, y in zip(a, b))
    return a == b
...
def build_init(
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        bound: dict[str, Any] = {}
        for spec, arg in zip(pos_specs, args):
...
def tree_flatten_factory(
    ...
    def unflatten(aux: Any, nodes: list[Any]) -> Any:
        ...
        values: dict[str, Any] = {}
        for name, value in zip(node_names_, nodes):
            values[name] = value
        for name, value in zip(static_names_, static_vals):
            values[name] = value
        for name, ref in zip(opaque_names_, opaque_vals):
