...
class MCState(VariationalState):
    ...
    def __init__(
        ...
        *,
        ...
        self._is_group_averaged = is_group_averaged
    ...
    @timing.timed
    @timeit(tag="SAMPLINNG", warn_ms=500, level=logging.DEBUG)
    def sample(
        self,
        *,
    ...
    @timing.timed
    def expect(self, O: Union[AbstractOperator, Sequence[AbstractOperator]]) -> Stats:
    ...
    def expect_and_grad(
        self,
        O: Union[AbstractOperator, Sequence[AbstractOperator]],
        *,
    ...
    @experimental
    def to_array_numpy(self, normalize: bool = True) -> np.ndarray:
        return to_array_numpy(
            self.hilbert,
            self._apply_fun,
            self.variables,
            normalize = normalize,
    ...
    def project(
        ...
        new MCState whose variables contain the trained parameters under the wrapper's `"base"`
        scope.
        ...
        # build wrapped model
        projected_model = wrap_model(self.model, **wrapper_kwargs)

        # instantiate new MCState with identical runtime knobs
        new_state = MCState(
            self.sampler,
            projected_model,
        ...
        # TRANSPLANT VARIABLES into wrapper's "base" scope
        import flax.core as flax_core
        tpl_vars = flax_core.unfreeze(new_state.variables)
        src_vars = flax_core.unfreeze(self.variables)

        def copy_leaves_like(template_subtree, source_subtree):
            return jax.tree_util.tree_map(
                lambda _t, _s: _s, template_subtree, source_subtree
            )

        # (A) parameters: params -> params/base
        if "params" not in tpl_vars or "base" not in tpl_vars["params"]:
            raise RuntimeError(
                "Projected model does not expose required params['base'] subtree."
            )
        tpl_vars["params"]["base"] = copy_leaves_like(
            tpl_vars["params"]["base"], src_vars["params"]
        )

        # (B) mutable collections: coll -> coll/base
        for coll_name, src_coll in src_vars.items():
            if coll_name == "params":
                continue
            if coll_name not in tpl_vars:
                tpl_vars[coll_name] = {}
            if "base" in tpl_vars[coll_name]:
                tpl_vars[coll_name]["base"] = copy_leaves_like(
                    tpl_vars[coll_name]["base"], src_coll
                )
            else:
                tpl_vars[coll_name]["base"] = src_coll

        # FINALISE variables
        new_state._variables = flax_core.freeze(tpl_vars)
        model_state, parameters = flax_core.pop(new_state._variables, "params")

        new_state.model_state = model_state
        new_state._parameters = parameters
        new_state._parameters_structure = jax.tree_util.tree_structure(parameters)

        # REUSE SAMPLER STATE
        new_state.sampler_state = self.sampler_state
        new_state._sampler_state_previous = getattr(
            self, "_sampler_state_previous", self.sampler_state
        )

        # REUSE SAMPLES IF REQUESTED
        if reuse_cached_samples and getattr(self, "_samples", None) is not None:
            new_state._samples = self._samples
        else:
            new_state.reset()
    ...
    def to_group_averaged(
        ...
        """
        Return a NEW MCState that evaluates the group-projected wavefunction built on top of
        ...
        # wrap current model with the projector (edge or full perms, characters optional)
        projected_model = wrap_model(
            self.model,
        ...
        # build a new MCState that uses the projected model, keeping runtime knobs identical
        new_state = MCState(
            self.sampler,
            projected_model,
            n_samples = self.n_samples,
            ...
            mutable = self.mutable,
            training_kwargs = dict(self.training_kwargs),
            is_group_averaged = True,
        )

        # transplant trained variables under the projector's 'base/' scope
        # work entirely on the private fields to avoid NetKet's structure checks
        # template produced by wrapped model.init(...)
        tpl_vars = flax_core.unfreeze(new_state.variables)

        # trained vanilla variables (params + mutables)
        src_vars = flax_core.unfreeze(self.variables)

        # helper: copy leaves while preserving container shape from template subtree
        def copy_leaves_like(template_subtree, source_subtree):
            return jax.tree_util.tree_map(lambda _t, _s: _s, template_subtree, source_subtree)

        # params -> params/base
        if "params" not in tpl_vars or "base" not in tpl_vars["params"]:
            raise RuntimeError("Projected model does not expose params['base'] subtree.")
        if "params" not in src_vars:
            raise RuntimeError("Source variables missing 'params' subtree.")

        tpl_vars["params"]["base"] = copy_leaves_like(
            tpl_vars["params"]["base"], src_vars["params"]
        )

        # copy all other mutable collections (e.g. batch_stats) under the
        # same collection -> ['base']
        for coll_name, src_coll in src_vars.items():
            if coll_name == "params":
                continue
            if coll_name not in tpl_vars:
            ...
            if "base" in tpl_vars[coll_name]:
                ...
                # if wrapper didn't pre-create the collection, just place the trained subtree there
                tpl_vars[coll_name]["base"] = src_coll

        # commit variables through aprivatea fields and refresh all caches consistently
        new_state._variables = flax_core.freeze(tpl_vars)

        # split into model_state and parameters like NetKet does
        ms, pars = flax_core.pop(new_state._variables, "params")
        new_state.model_state = ms
        new_state._parameters = pars
        new_state._parameters_structure = jax.tree_util.tree_structure(pars)

        # reuse sampler state so chains continue from the same positions
        new_state.sampler_state = self.sampler_state
        new_state._sampler_state_previous = getattr(
            self, "_sampler_state_previous", self.sampler_state
        )

        # reuse cached samples if requested (identity-only perms -> exactly identical expectation)
        if reuse_cached_samples and getattr(self, "_samples", None) is not None:
            new_state._samples = self._samples
        else:
            new_state.reset()
    ...
    def __str__(self):
        return (
            ...
            + f"is_group_averaged = {self._is_group_averaged}, "
...
# TODO: add a field for current sampler state and deserialise two vstates for each
# serialization
...
def deserialize_MCState(vstate, state_dict, force_load_mpi: bool = False):
    import copy
    ...
    # quick backwards compatibility fix with previously saved states in dict format
    if isinstance(state_dict["variables"], dict):
        deserialising_func = serialization.from_state_dict
    else:
        deserialising_func = serialization.from_bytes

    vars = jax.tree_util.tree_map(
        jnp.asarray,
        deserialising_func(vstate.variables, state_dict["variables"]),
    )
...
def cast_MCState(vstate, state_dict, n_samples):

    new_vstate = NKMCState(
        vstate.sampler,
        vstate.model,
        n_samples = n_samples,
    )
