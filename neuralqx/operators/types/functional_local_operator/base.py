...
from neuralqx.utils.errors import InvalidMelsFunctionError
from .helpers import canonicalize_input
from .helpers import _multiply_operators
from .helpers import cast_operator_matrix_dtype
from .convert import local_operators_to_pauli_strings
from .compile_helpers import _infer_mel_dtype
from ._specialised_complex import _multiply_operators as _s_multiply_operators
...
class LocalOperatorBase(DiscreteOperator, DiscreteOperatorNK):
    ...
    def __init__(
        ...
        *,
        ...
        mels_func: Callable = lambda x: x,  # dev: do nothing to the mels
        specialised: bool = False,
        force_hermitian: bool = None,
    ):
        r"""
        ...
        # dev: save the mels func
        #  this butchered approach is because we need to be fine with whatever function the user
        #  passes, this already fails for numpy.ufunc's, this butchering is to ensure it is a Python
        #  function
        if (
            not isinstance(mels_func, types.LambdaType)
            or not mels_func.__name__ == "<lambda>"
        ):
            raise InvalidMelsFunctionError

        # save the raw lambda function so that we can use it in .copy()
        self._mels_func = mels_func

        # the gccf's are numba jitted, so we have to numba compile our mels_func so that we can pass
        # it to them without issue
        self._njit_mels_func = numba.jit(nopython=True)(mels_func)

        # we choose the multiplication of operators based on the flag
        if specialised:
            self._multiply_operators = _s_multiply_operators
        else:
            self._multiply_operators = _multiply_operators

        # this also needs to be saved as an instance attribute to correctly .copy()
        self._specialised = specialised

        # a flag to bypass the hermiticity check
        self._force_hermitian = force_hermitian

        # store the expected mels dtype for JAX compatibility
        self._mels_dtype = _infer_mel_dtype(self._mels_func, self._dtype)
    ...
    def is_hermitian(self) -> bool:
        ...
        # check if we bypass the hermiticity check
        if self._force_hermitian is not None:

            self._is_hermitian = self._force_hermitian
    ...
    def __iadd__(self, other):
        if isinstance(other, LocalOperatorBase) or isinstance(other, NKLOPB):
        ...
        if isinstance(other, DiscreteOperator) or isinstance(other, DiscreteOperatorNK):
            ...
            if np.abs(other) <= self.mel_cutoff:
                ...
                for key in self._operators_dict:
                    # dev: should this be specialised or not?
                    self._operators_dict[key] = other * self._operators_dict[key]

                    # print(f"other = {other}")
                    # self._operators_dict[key] = _broadcastMultiplyReal(
    ...
    def __imatmul__(self, other):
        if not isinstance(other, LocalOperatorBase) and not isinstance(other, NKLOPB):
            return NotImplemented
    ...
    def _op_imatmul_(self, other: "LocalOperatorBase") -> "LocalOperatorBase":
        ...
        if np.abs(α) > self.mel_cutoff:
            for aon, op in B_op_dict.items():
                # dev: this doesnt need to be specialised, as we are multiplying the current op's
                #  constant with all ops of the other?
